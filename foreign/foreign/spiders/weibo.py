...
resp = session.get(url_prelogin).content
req = str(resp)
json_data = re.search(r'\((.*)\)',req).group(1)
...
# calc su，第一步加密用户名
so = urllib.quote(username)
# sos =  bytes(so,encoding='utf-8')
su = base64.b64encode(so)
...
login_urls = ''.join(login_url)
# # print login_url
resp = session.get(login_urls,verify=False)
# # print resp.content
